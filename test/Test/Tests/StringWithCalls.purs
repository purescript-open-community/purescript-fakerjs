module Test.Tests.StringWithCalls where
...
mkTest :: String -> Array (Either String FunctionCall) -> Spec Unit
...
mkTestShouldFail
  ...
  -> Spec Unit
mkTestShouldFail input expectedErrors = it ("fails to parse input: " <> show input) do
  let fs = unsafePartial $ NonEmptyString.unsafeFromString
  let input' = fs input
  case parseStringWithCalls input' of
    Right result ->
